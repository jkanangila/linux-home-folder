...
local function get_environment_info()
  ...
  if path_separator == "/" then
    ...
    if vim.fn.has "mac" == 1 or vim.fn.has "macunix" == 1 then detected_os = "macos" end
  ...
  if detected_os == "linux" or detected_os == "macos" then
    ...
    if prefix and string.find(prefix, "com.termux") then return "linux", "termux" end

    local android_root = os.getenv "ANDROID_ROOT"
    local android_data = os.getenv "ANDROID_DATA"
    local path_env = os.getenv "PATH" or ""
    if android_root or android_data or string.find(path_env, "com.termux") then return "linux", "android_chroot" end

    local version_file = io.open("/proc/version", "r")
    if version_file then
      local version_info = version_file:read("*all"):lower()
      version_file:close()
      if string.find(version_info, "microsoft") or string.find(version_info, "wsl") then return "linux", "wsl" end
    end
  end
